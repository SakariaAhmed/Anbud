...
export function buildCustomerAnalysisPrompt() {
  return buildPromptTemplate({
    ...
    rules: [
      ...
      "prioritized_requirements skal trekke frem de viktigste kravene og prioriteringene, ikke alle krav. Prioriter absolutte krav med konsekvens for avvisning, sikkerhet eller leveranseevne foran generelle forbedringer. Bevar krav-ID og terskel; relaterte delkrav kan samles i ett tydelig punkt uten å miste vilkår.",
      ...
      "Bevar konkrete drifts- og overleveringskrav som oppbevaringstid, opplæring, driftsdokumentasjon og akseptanse når de finnes. Plasser dem samlet i designretning eller gjennomføring uten å erstatte dem med generelle ord som 'sikker drift' eller 'god overlevering'.",
...
export function buildSolutionEvaluationPrompt(
  ...
  return buildPromptTemplate({
    ...
    rules: [
      ...
      "Forbedringsråd og rewrite_suggestions skal bevare dokumenterte avvik, manglende prising og nødvendige leverandørbekreftelser. Ikke anbefal å fjerne et reelt forbehold eller gjøre ubekreftet leveranse til et ubetinget løfte for å øke score. Beskriv hva leverandøren må beslutte, dokumentere eller endre før avviket kan lukkes; et tydelig kundekrav endres ikke av dette.",
      ...
      "Kravfunn skal kopiere full_reference fra den relevante kravdetaljen, inkludert dokumentnavn. Samme lokale krav-ID eller sitat kan finnes i flere dokumenter. For seksjonsfunn skal document_findings.reference være en eksakt referanse fra dokumentstrukturen eller nærmeste kildeindikasjon, for eksempel side, seksjon, tabell, rad, ark eller overskrift. Ikke bruk generiske referanser som 'dokumentet' eller 'løsningen'.",
  ...
  if (artifactType === "forbedret_kravsvar") {
    return buildPromptTemplate({
      ...
      ...PROJECT_SOURCE_EVIDENCE_RULES,
        ...
        "Kundens krav er ikke bevis på leverandørens eksisterende dekning. Hvis løsningsgrunnlaget avviser, utelater prising eller utsetter bekreftelse, merk svaret som Ubekreftet forutsetning – må avklares, og skill foreslått ny leveranse fra dokumentert leveranse. Bevar uttrykkelige avvik selv om andre kilder gjentar kundekravet.",
  ...
  if (artifactType === "losningsutkast") {
    return buildPromptTemplate({
      ...
      ...PROJECT_SOURCE_EVIDENCE_RULES,
        ...
        "Kontroller løsningsdokumentet mot kundekravene også når ingen løsningsvurdering finnes. For hvert dokumentert avvik: oppgi kravreferansen og dagens avvik kort, og merk den konkrete endringen som foreslått leveranse som krever leverandørens bekreftelse. Bruk denne merkingen også i 'Hva som inngår'; ikke gjør forslag til ubetingede 'Vi leverer'-løfter. Teksten alene er ikke bevis på at avviket er lukket.",
  ...
  if (artifactType === "gjennomforing_og_risiko") {
    return buildPromptTemplate({
      ...
      ...PROJECT_SOURCE_EVIDENCE_RULES,
        ...
        "Et uttrykkelig bindende kundekrav skal ligge fast i planen. Ved leverandøravvik skal beslutningspunktet gjelde hvordan leverandøren lukker og dokumenterer avviket; ikke spør om kunden kan godta en svakere leveranse eller et unntak som kilden ikke åpner for.",
...
export function buildChatPrompt() {
  return buildPromptTemplate({
    ...
    rules: [
      ...
      "Ved dokumenterte avvik skal forslag til avklaringer gjelde leverandørens konkrete retting og bevis. Ikke gjør et uttrykkelig bindende kundekrav til et åpent spørsmål om kunden kan godta svakere dekning eller udokumenterte unntak.",
