...
import { accountedCostUpperBound, validatedBudgetLimit } from "./budget.mjs";
...
const costUpperBound = budget.requests.reduce((sum, row) => sum + Math.round(accountedCostUpperBound(row) * 1e6), 0) / 1e6;
const comparisons = [];
const complete = (run) => {
...
const index = {
  ...
  additionalJudgePreflight: { verification: "verification/judge-preflight-gpt54-v1-total.json", scope: "Historical offline preparation against the 467-row ledger: two GPT-5.4 judges reserved 0.327356 USD in total. Both matched existing Mini payloads except model. Subsequent authorized execution is recorded separately; this preflight is not itself a verdict or quality acceptance." },
  additionalIndependentJudges: read("verification/two-gpt54-judges-reconciliation-v1.json"),
  authorizedBudgetRefill: { authorization: "verification/api-budget-refill-authorization-v1.json", testPlan: "verification/api-budget-refill-test-plan-v1.json", archive: "api-budget-before-authorized-refill-v1.json", scope: "Explicit user refill grants at most 14 new USD from prior conservative cost 13.393718 USD. Same ledger, all 469 prior rows preserved, cumulative ceiling 27.393718 USD. Earlier budget preflight gaps remain historical evidence; fresh live workflow verification is now funded but incomplete." },
  completeDevelopmentWorkflowPair: { verification: "verification/perfect-workflow-v1/comparison.json", sourceReview: "verification/perfect-workflow-v1/source-review.json", scope: "One actual queued worker pair completed with new drafts and evaluations persisted against the correct artifact IDs. 113.421 to 110.162 seconds, not significant/statistically reliable speed proof; self-scores 88/84 are not independent quality acceptance. Original fixtures unchanged, own DBs/containers removed. No HTTP/Azure/holdout claim." },
  judgeContractQualification: "verification/v3-judge-contract-qualification-v1.json",
  firstQuestionChatDevelopment: { verification: "verification/chat-first-question-v1/comparison.json", scope: "Four development pairs, including distinct empty/current-message input variants and a long-source question. Avoidable rewrite removed; all observed pairs faster. Two advisory noninferiority failures preserved. Long-source omissions and citation precision remain unresolved; no universal quality or holdout claim." },
  chatSourceEvidenceCorrection: { verification: "verification/chat-evidence-v1/comparison.json", scope: "Subsequent fix preserves selected chunks, prioritizes question topics and retains all16 locators through repository/API/UI. Long development omissions corrected; two new locator-aware advisory judge wins. Both full answers slower; no universal speed/quality acceptance. Original browser HTTP cleanup500 and separate local DB cleanup retained." },
  systemArtifactContextAndHolisticTrial: { verification: "verification/holistic-artifact-v1/comparison.json", scope: "Full scored artifact and correct artifact/analysis priority. Actual long counterexample exposes then fixes lost MFA exemption; overall advisory judge prefers baseline and is preserved. Isolated same-input GPT-5.4/Terra pair gives no speed gain; GPT-5.4 retained. Same imported coverage replayed; not a fresh complete workflow or holdout test." },
  rejectedCompactFindingTrials: { comparisons: ["verification/compact-findings-v2/comparison.json", "verification/compact-findings-v3/comparison.json"], scope: "Compact medium pair gave no observed speed improvement. Reverse-order repetition failed on Norwegian quote delimiters; original failure retained, remainder stopped. Compact application code removed after trials; patch/tests archived. Generic quote correction retained and verified through original full-finding owner path. Ambiguous matching across duplicate local IDs/identical evidence remains a separate existing risk." },
  rejectedLowReasoningTrial: { verification: "verification/holistic-reasoning-v2/comparison.json", scope: "Same compact prompts/code/source, only medium versus low differs. One pair 86.489 to58.701 seconds, advisory noninferiority failed. Both retain unconfirmed-commitment advice defects. No app model/reasoning change, p95, complete-workflow or holdout acceptance." },
