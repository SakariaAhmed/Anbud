...
if (paired && !label) throw new Error("Paired replacement fixtures need an explicit new label.");
const output = path.join(root, `output/speed-quality-2026-09-08/${forWrites ? "write" : "read"}-fixtures${label ? `-${label}` : ""}.json`);
...
let apiUrl = env.DATA_API_URL;
async function db(route, method = "GET", body) {
  const response = await fetch(`${apiUrl}/${route}`, { method, headers: { authorization: `Bearer ${env.DATA_API_SERVICE_ROLE_KEY}`, "content-type": "application/json", prefer: "return=representation" }, body: body === undefined ? undefined : JSON.stringify(body) });
...
for (const [size, count] of [["small", 8], ["large", 400]]) {
  for (let index = 0; index < (forWrites ? 1 : 3); index++) {
    ...
    const evaluationRow = { id: randomUUID(), project_id: id, evaluation_provenance_mode: "document_only", result_json: encryptJson(result), created_at: timestamp, updated_at: timestamp };
    const artifactRows = ["losningsutkast", "bilag1_rekonstruksjon", "forbedret_kravsvar", "tilbudsstrategi", "verdiargumentasjon", "anbefalt_arkitektur", "gjennomforing_og_risiko"].map((type) => ({ id: randomUUID(), artifact_type: type }));
    for (const target of paired ? ["http://127.0.0.1:55440", "http://127.0.0.1:55443"] : [env.DATA_API_URL]) {
      apiUrl = target;
      const states = await db("artifact_source_state?singleton=eq.true");
      await db("projects", "POST", { id, owner_id: env.APP_ADMIN_PRINCIPAL_ID, client_name: "Fiktiv målekunde", title: `Måleprosjekt ${size} ${index + 1} ${label}`, created_at: timestamp, updated_at: timestamp, last_activity_at: timestamp });
      // The owner-membership trigger uses its own now(). Align this fixture's
      // acceptance time instead of dropping it from HTTP comparisons.
      await db(`project_memberships?project_id=eq.${id}&principal_id=eq.${env.APP_ADMIN_PRINCIPAL_ID}`, "PATCH", { accepted_at: timestamp });
      const evaluation = (await db("solution_evaluations", "POST", evaluationRow))[0];
      const dependency = await db("rpc/raw_artifact_solution_evaluation_dependency", "POST", { p_project_id: id });
      // This field's persistence contract is plaintext Markdown. Only result_json
      // and input_snapshot use encryptJson; encryptString here produced invalid
      // historical fixtures, which must be preserved separately.
      await db("generated_artifacts", "POST", artifactRows.map((artifact) => ({ ...artifact, project_id: id, title: "Syntetisk artefakt", content_markdown: text.repeat(count), created_at: timestamp, updated_at: timestamp, artifact_version: 1, input_artifact_source_revision: 0, input_service_library_revision: states[0].service_library_revision, used_solution_evaluation: true, input_solution_evaluation_id: evaluation.id, input_solution_evaluation_updated_at: evaluation.updated_at, input_solution_evaluation_hash: dependency.content_hash })));
      if (forWrites) {
      ...
      const roundTrip = await db(`generated_artifacts?project_id=eq.${id}&select=content_markdown`);
      if (roundTrip.length !== 7 || roundTrip.some((a) => a.content_markdown !== text.repeat(count))) throw new Error("Fixture content contract failed.");
    }
