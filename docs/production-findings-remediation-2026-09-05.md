...
Long Bilag export additionally split a heading at the page boundary. Pagination now measures the actual html2canvas clone and chooses a boundary before headings, keeping them with their content. The two-page Bilag PDF was downloaded locally and both pages visually checked with production CSS: complete heading, all content and full confidence table. Targeted pagination tests passed; full CI repeats the suite on this final patch.

### Final diagram rendering verification

The full browser rendering pipeline exposed two additional Mermaid 11.16 regressions: the deprecated flowchart-level `htmlLabels` option was overridden, and the generated theme's internal gradient URLs caused the entire stylesheet to be removed. Use root-level SVG labels and permit only simple local fragment references in rendered CSS. Strict source filtering, foreign-object removal, and rejection of external URLs remain in place. The exact saved production fallback now renders all seven node labels, groups and connecting arrows through the actual sanitizer/enhancement helpers; malicious CSS cases are covered by regression tests.
